...
@Component({
  tag: 'multiselect-buttons-two',
  styleUrl: './multiselect.css',
  shadow: false
})
export class MultiselectButtonsV2 {
  /**
  ...
  // save timeout ID for status message
  ...
  componentDidUpdate() {
    ...
    if (this.callRemoveFocus === true) {
      this.removeFocusRef && this.removeFocusRef.focus();
      this.callRemoveFocus = false;
    }
    ...
    return ([
      ...
      <ul class="selected-options" id={`${this.htmlId}-selected`}>
        ...
        {selectedOptions.map((option, i) => {
          return (
            <li>
              <button class="remove-option" type="button" id={`${htmlId}-remove-${i}`} aria-labelledby={`${htmlId}-remove ${htmlId}-remove-${i}`} ref={activeRemoveIndex === i ? (el) => this.removeFocusRef = el : null} onClick={() => { this.removeOption(i, true); }}>
                {option.name} 
              </button>
            </li>
          )
      ...
      <div class="visuallyHidden" role="status">{liveStatus}</div>,
  ...
  private removeOption(index: number, updateFocus = false) {
    ...
    this.updateStatusMessage(`removed ${removedOption.name}`);
  }
  ...
  private updateOption(index: number) {
    ...
    if (isSelected) {
      ...
      this.updateStatusMessage(`added ${option.name}`);
